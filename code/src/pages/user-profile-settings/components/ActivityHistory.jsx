...
const ActivityHistory = ({ user, onDataChange }) => {
  ...
  const activityData = [
    {
      ...
      description: 'Căn hộ hiện đại Downtown - 123 Main Street',
      timestamp: '2024-01-15T14:30:00Z',
      metadata: { propertyId: 'prop_123', price: '$425,000' }
    },
    {
      ...
      description: 'Tìm "Nhà 3 phòng ngủ dưới $500K"',
      timestamp: '2024-01-15T10:15:00Z',
      metadata: { query: '3 bedroom houses under $500K', results: 23 }
    },
    {
      ...
      description: 'Nhà Suburban hấp dẫn - 456 Oak Avenue',
      timestamp: '2024-01-14T16:45:00Z',
      metadata: { propertyId: 'prop_456', price: '$675,000' }
    },
    {
      ...
      type: 'message_sent',
      title: 'Gửi Tin Nhắn',
      description: 'Hỏi về lịch tham quan bất động sản',
      timestamp: '2024-01-14T09:20:00Z',
      metadata: { recipientType: 'agent', propertyId: 'prop_789' }
    },
    {
    ...
    },
    {
      ...
      type: 'saved_search',
      title: 'Tìm kiếm Lưu',
      description: 'Căn hộ Downtown - khoảng $300K-$500K',
      timestamp: '2024-01-12T13:15:00Z',
      metadata: { searchId: 'search_123', frequency: 'daily' }
    },
    {
      ...
      description: 'Đăng nhập từ Chrome trên MacOS',
      timestamp: '2024-01-12T08:45:00Z',
      metadata: { device: 'Chrome on MacOS', ip: '192.168.1.1' }
    },
    {
      ...
      type: 'property_view',
      title: 'Xem Bất Động Sản',
      description: 'Biệt thự sang trọng ven hồ - 789 Lakeside Drive',
      timestamp: '2024-01-11T15:20:00Z',
      metadata: { propertyId: 'prop_789', price: '$1,250,000' }
    }
  ];
  ...
  const periodOptions = [
    ...
    { value: '90days', label: '3 tháng gần đây' },
    { value: '1year', label: 'Năm qua' },
    { value: 'all', label: 'Tất cả thời gian' }
  ];

  const typeOptions = [
    { value: 'all', label: 'Tất cả hoạt động' },
    { value: 'property_view', label: 'Xem Bất Động Sản' },
    { value: 'search', label: 'Tìm kiếm' },
    { value: 'favorite_added', label: 'Yêu thích' },
    { value: 'message_sent', label: 'Tin nhắn' },
  ...
  const handleExportData = (format) => {
    ...
    if (format === 'json') {
      dataStr = JSON.stringify(exportData, null, 2);
      filename = `lich_su_hoat_dong-${new Date()?.toISOString()?.split('T')?.[0]}.json`;
      mimeType = 'application/json';
    } else if (format === 'csv') {
      const headers = ['Ngày', 'Loại', 'Tiêu đề', 'Mô tả'];
      const csvData = filteredActivities?.map(activity => [
        new Date(activity.timestamp)?.toLocaleString(),
        activity?.type,
        activity?.title,
      ...
      filename = `lich_su_hoat_dong-${new Date()?.toISOString()?.split('T')?.[0]}.csv`;
      mimeType = 'text/csv';
    }
      ...
      <div className="px-6 py-4 border-b border-border">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-sm text-text-secondary mt-1">
              Theo dõi các hoạt động và hành động tài khoản gần đây
            </p>
          </div>
