...
const SimilarProperties = ({ properties = [] }) => {
  ...
  // Cập nhật hình ảnh mới nếu hình cũ lỗi thời
  const updatePropertyImages = (property) => {
    ...
    const hasOutdatedImages = property?.images?.some(img => 
      !img?.includes('w=800') || img?.includes('very-old-image-url') || img?.includes('outdated-photo')
    );
    ...
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-lg font-semibold text-text-primary mb-2">
          Không có bất động sản tương tự
        </h3>
        <p className="text-text-secondary">
          Hiện tại không tìm thấy bất động sản nào tương tự.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-text-primary">Bất động sản tương tự</h2>
          <p className="text-text-secondary">Các bất động sản bạn có thể quan tâm</p>
        </div>
          ...
          <div className="flex md:hidden justify-center space-x-2 mt-4">
            {Array.from({ length: properties.length }).map((_, index) => (
              <button
                ...
                aria-label={`Đi tới bất động sản ${index + 1}`}
              />
            ))}
          </div>
        )}
