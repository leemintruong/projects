...
const AnalyticsSection = () => {
  ...
  const performanceData = [
    { month: 'Tháng 1', listings: 12, leads: 45, sales: 5 },
    { month: 'Tháng 2', listings: 15, leads: 52, sales: 7 },
    { month: 'Tháng 3', listings: 18, leads: 48, sales: 6 },
    { month: 'Tháng 4', listings: 20, leads: 62, sales: 9 },
    { month: 'Tháng 5', listings: 24, leads: 68, sales: 8 },
    { month: 'Tháng 6', listings: 22, leads: 55, sales: 10 }
  ];

  const revenueData = [
    { month: 'Tháng 1', revenue: 45000 },
    { month: 'Tháng 2', revenue: 52000 },
    { month: 'Tháng 3', revenue: 48000 },
    { month: 'Tháng 4', revenue: 62000 },
    { month: 'Tháng 5', revenue: 68000 },
  ...
  const handleExport = (format) => {
    console.log(`Xuất dữ liệu dưới dạng ${format}`);
    // TODO: Viết logic export thật
  };
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="relative">
              <select
                ...
                defaultValue=""
              >
                <option value="" disabled>Xuất dữ liệu</option>
                <option value="pdf">Xuất PDF</option>
                <option value="excel">Xuất Excel</option>
                <option value="csv">Xuất CSV</option>
              </select>
            </div>
          </div>
        </div>
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={performanceData}>
                ...
                <Line type="monotone" dataKey="listings" stroke="#2563EB" strokeWidth={2} dot={{ fill: '#2563EB', r: 4 }} name="Danh sách" />
                <Line type="monotone" dataKey="leads" stroke="#0EA5E9" strokeWidth={2} dot={{ fill: '#0EA5E9', r: 4 }} name="Khách hàng tiềm năng" />
                <Line type="monotone" dataKey="sales" stroke="#059669" strokeWidth={2} dot={{ fill: '#059669', r: 4 }} name="Giao dịch" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        )}
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={revenueData}>
                <CartesianGrid strokeDasharray="3 3" stroke="#E2E8F0" />
                <XAxis dataKey="month" stroke="#64748B" fontSize={12} />
                <YAxis stroke="#64748B" fontSize={12} tickFormatter={formatRevenue} />
                <Tooltip contentStyle={{
                  backgroundColor: '#FFFFFF',
                  border: '1px solid #E2E8F0',
                  borderRadius: '8px',
        ...
        <div className="mt-6 flex flex-wrap gap-4">
          {activeChart === 'performance' && (
            <>
              <div className="flex items-center space-x-2">
                <div className="w-3 h-3 bg-primary rounded-full"></div>
                <span className="text-sm text-text-secondary">Danh sách</span>
              </div>
              <div className="flex items-center space-x-2">
                <div className="w-3 h-3 bg-accent rounded-full"></div>
                <span className="text-sm text-text-secondary">Khách hàng tiềm năng</span>
              </div>
              <div className="flex items-center space-x-2">
                <div className="w-3 h-3 bg-success rounded-full"></div>
                <span className="text-sm text-text-secondary">Giao dịch</span>
              </div>
            </>
          )}
          {activeChart === 'revenue' && (
            <div className="flex items-center space-x-2">
              <div className="w-3 h-3 bg-warning rounded-full"></div>
              <span className="text-sm text-text-secondary">Doanh thu hàng tháng</span>
            </div>
          )}
        </div>
      </div>
