...
const BusinessProfile = ({ user, onDataChange }) => {
  ...
  const availabilityOptions = [
    ...
    { value: 'weekends', label: 'Chỉ cuối tuần' },
    { value: 'evenings', label: 'Chỉ buổi tối' }
  ];

  const predefinedSpecializations = [
    'Bất động sản cao cấp', 'Người mua lần đầu', 'Bất động sản đầu tư',
    'Bất động sản thương mại', 'Bất động sản ven sông/ven biển', 'Dự án mới xây',
    'Bất động sản lịch sử', 'Căn hộ chung cư', 'Bất động sản đa gia đình'
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-surface rounded-lg shadow-elevation-1">
        <div className="px-6 py-4 border-b border-border">
          ...
          <p className="text-sm text-text-secondary mt-1">
            Quản lý thông tin giấy phép hành nghề bất động sản của bạn
          </p>
        </div>
      ...
      <div className="bg-surface rounded-lg shadow-elevation-1">
        <div className="px-6 py-4 border-b border-border">
          <h2 className="text-xl font-semibold text-text-primary font-heading">
            Thông Tin Công Ty Môi Giới
          </h2>
          <p className="text-sm text-text-secondary mt-1">
            Thông tin về công ty môi giới của bạn
          </p>
        </div>
      ...
      <div className="bg-surface rounded-lg shadow-elevation-1">
        <div className="px-6 py-4 border-b border-border flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-text-secondary mt-1">
              Xác định khu vực mà bạn cung cấp dịch vụ
            </p>
          </div>
          <button
            onClick={() => setShowMapModal(true)}
        ...
        <div className="p-6">
          ...
          <div className="flex space-x-2">
            <input
              ...
              onChange={(e) => setNewServiceArea(e?.target?.value)}
              onKeyPress={(e) => e?.key === 'Enter' && handleArrayAdd('serviceAreas', newServiceArea, setNewServiceArea)}
              placeholder="Thêm khu vực phục vụ"
              className="flex-1 px-4 py-2 border border-border rounded-md focus:border-border-focus focus:ring-2 focus:ring-primary-500 transition-all duration-200"
            />
            <button
              onClick={() => handleArrayAdd('serviceAreas', newServiceArea, setNewServiceArea)}
      ...
      <div className="bg-surface rounded-lg shadow-elevation-1">
        <div className="px-6 py-4 border-b border-border">
          ...
          <p className="text-sm text-text-secondary mt-1">
            Nêu bật các lĩnh vực chuyên môn của bạn
          </p>
        </div>
        {/* ... phần specializations và professional details tiếp tục Việt hóa tương tự ... */}
      </div>
      ...
      {showMapModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-modal flex items-center justify-center p-4">
          <div className="bg-surface rounded-lg shadow-xl max-w-4xl w-full max-h-[80vh] overflow-hidden">
            <div className="px-6 py-4 border-b border-border flex items-center justify-between">
              <h3 className="text-lg font-semibold text-text-primary">Chọn Khu Vực Phục Vụ</h3>
              <button
                onClick={() => setShowMapModal(false)}
                className="text-text-secondary hover:text-text-primary"
              >
            ...
            <div className="p-6">
              <div className="h-96 bg-secondary-100 rounded-lg flex items-center justify-center">
                <div className="text-center">
                  <Icon name="Map" size={64} className="text-secondary-300 mx-auto mb-4" />
                  <p className="text-text-secondary">
                    Bản đồ tương tác để chọn khu vực phục vụ sẽ được hiển thị tại đây
                  </p>
                  <p className="text-sm text-text-secondary mt-2">
                    Tích hợp dịch vụ bản đồ như Google Maps hoặc Mapbox
                  </p>
                </div>
              </div>
            </div>
