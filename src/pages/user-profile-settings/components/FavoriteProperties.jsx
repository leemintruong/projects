...
const FavoriteProperties = ({ user, onDataChange }) => {
  const [favoriteProperties, setFavoriteProperties] = useState([
    {
      id: '1',
      title: 'Căn hộ hiện đại trung tâm thành phố',
      address: '123 Main Street, Unit 4B',
      price: '425,000,000 VND',
      beds: 2,
      baths: 2,
      sqft: 1200,
    ...
    {
      id: '2',
      title: 'Ngôi nhà ven đô duyên dáng',
      ...
      lastPriceChange: '-25,000,000 VND'
    },
    {
      id: '3',
      title: 'Biệt thự sang trọng ven hồ',
      address: '789 Lakeside Drive',
      price: '1,250,000,000 VND',
      beds: 5,
      baths: 4,
      sqft: 3800,
  ...
  const handleRemoveFavorite = (propertyId) => {
    if (window.confirm('Xóa bất động sản này khỏi danh sách yêu thích?')) {
  ...
  const handleBulkRemove = () => {
    ...
    if (window.confirm(`Xóa ${selectedProperties?.length} bất động sản khỏi danh sách yêu thích?`)) {
  ...
  const handleShare = () => {
    if (selectedProperties?.length === 0) {
      alert('Vui lòng chọn bất động sản để chia sẻ');
      return;
    }
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'active':
        return <span className="bg-success-100 text-success px-2 py-1 rounded-full text-xs font-medium">Đang mở bán</span>;
      case 'price_drop':
        return <span className="bg-warning-100 text-warning px-2 py-1 rounded-full text-xs font-medium">Giảm giá</span>;
      case 'sold':
        return <span className="bg-secondary-100 text-text-secondary px-2 py-1 rounded-full text-xs font-medium">Đã bán</span>;
      default:
        return null;
    }
  };
        ...
        {selectedProperties?.length > 0 && (
          <div className="mt-4 flex items-center justify-between p-3 bg-primary-100 rounded-lg">
            <span className="text-sm font-medium text-primary">
              {selectedProperties?.length} bất động sản được chọn
            </span>
        ...
        {filteredProperties?.length === 0 ? (
          <div className="text-center py-8">
            ...
            <h3 className="text-lg font-medium text-text-primary mb-2">
              {favoriteProperties?.length === 0 ? 'Chưa có bất động sản yêu thích' : 'Không có bất động sản nào phù hợp'}
            </h3>
            <p className="text-text-secondary">
              {favoriteProperties?.length === 0 
                ? 'Bắt đầu tìm kiếm và lưu những bất động sản bạn yêu thích ở đây.' 
                : 'Thử điều chỉnh bộ lọc để xem thêm bất động sản.'}
            </p>
          </div>
        ) : (
          <>
                ...
                <div key={property?.id} className="border border-border rounded-lg overflow-hidden hover:shadow-elevation-2 transition-shadow duration-200">
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between text-sm text-text-secondary">
                      <span>Người môi giới: {property?.listingAgent}</span>
                      <span>Ngày thêm: {property?.dateAdded}</span>
                    </div>
                  </div>
                </div>
              ))}
      ...
      {showShareModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-modal flex items-center justify-center p-4">
          <div className="bg-surface rounded-lg shadow-xl max-w-md w-full p-6">
            <h3 className="text-lg font-semibold text-text-primary mb-4">Chia sẻ bất động sản</h3>
            
            <p className="text-text-secondary mb-4">
              Chia sẻ {selectedProperties?.length} bất động sản đã chọn qua:
            </p>
